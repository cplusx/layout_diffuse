...
class PLMSSampler(object):
    ...
    def plms_sampling(
        ...
        print(f"Running PLMS Sampling with {total_steps} timesteps. Note this is only for baseline")
    ...
    def p_sample_plms(
        ...
        def get_model_output(x, t):
            e_t = self.model(torch.cat([x, y_cond], dim=1), t, **model_kwargs)
            if (uncondition_model_kwargs is not None) and (unconditional_guidance_scale != 1.):
                e_t_uncond = self.model(torch.cat([x, y_cond], dim=1), t, **uncondition_model_kwargs)
